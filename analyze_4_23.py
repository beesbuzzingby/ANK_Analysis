...
def analysis_dat(cooc_dat, succ_dat, feature, feature2, f_name, f_name2):
    ...
    df.to_csv("regression_dat_2.csv")
...
''' 'R': list of valid responses used to create response pairs '''
R = calc_cooccurrence()
...
''' df: dataframe of all the data necessary for analysis
    columns: 
        Item- The item both responses belong to (A, B, C, D, or E)
        Target- The value each response must be equal to (6, 16, 59, or 12)
        Response_1- The first response in a response pair
        Response_2- The second response in a response pair
        Overall- The frequency both responses in a pair were generated by the same participant 
            (aka "Overall Cooccurrence)
        Successive- The frequency both responses in a pair were generated by the same participant 
            in immediate succession (aka "Successive Cooccurrence)
        Commutativity- Whether or not a pair is commutative (0 for no, 1 for yes) 
        Add_Decomp- Whether or not a pair is additively decomposed (0 for no, 1 for yes)'''
